...
export default async function DashboardPage() {
  ...
  const effectiveClinicId = await getEffectiveClinicId(
    session!.user.role,
  ...
  // Add this in case client dont want to show patients that are hidden
  const hiddenFilter = { AND: { NOT: { firstName: { startsWith: "#" } } } };
  const [
    todayCount,
    patientCount,
    monthCount,
    ...
    prisma.appointment.findMany({
      where: { ...clinicFilter, startTime: { gte: todayStart, lte: todayEnd } },
      include: {
        ...
        <Card>
          ...
          <CardContent>
            {todayAppointments.length === 0 ? (
              <EmptyState
                icon={Clock}
                message="Nema zakazanih termina za danas."
              />
            ) : (
